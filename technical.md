...
- 예를 들어, 다음과 같은 `.env` 파일을 사용할 수 있습니다:
...
이 `.env` 파일은 데이터베이스 연결 정보나 스프링 프로파일과 같은 환경 변수를 정의합니다. 이를 `docker-compose.yml` 파일에서 참조하면, 환경에 맞게 설정을 쉽게 변경할 수 있습니다. 예를 들어, `docker-compose.yml` 파일에서 다음과 같이 참조할 수 있습니다:
...
volumes:
  ...
  - ./spring/config:/app/config
```

위 설정에서는 호스트의 `spring/logs` 디렉토리를 컨테이너 내의 `/app/logs` 디렉토리와 연결하고, `spring/config` 디렉토리를 `/app/config` 디렉토리와 연결합니다. 이를 통해 컨테이너가 종료되더라도 로그와 설정 파일은 호스트에 영구적으로 저장됩니다. 이는 개발 및 운영 환경에서 데이터를 안전하게 보관하고, 설정 파일을 쉽게 관리할 수 있게 해줍니다.
